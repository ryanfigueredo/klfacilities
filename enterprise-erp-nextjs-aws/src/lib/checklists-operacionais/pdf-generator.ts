...
try {
  ...
  console.warn('Fontkit não disponível, fontes customizadas não funcionarão:', error);
}
...
async function embedFigtreeFont(pdfDoc: PDFDocument): Promise<{ regular: any; bold: any }> {
  try {
    // Tentar carregar Figtree TTF diretamente do Google Fonts
    // URL direta para o arquivo TTF da fonte Figtree Regular
    const regularUrl = 'https://github.com/google/fonts/raw/main/ofl/figtree/Figtree-Regular.ttf';
    const boldUrl = 'https://github.com/google/fonts/raw/main/ofl/figtree/Figtree-SemiBold.ttf';
    ...
    try {
      const regularPath = path.join(process.cwd(), 'public', 'fonts', 'Figtree-Regular.ttf');
      const boldPath = path.join(process.cwd(), 'public', 'fonts', 'Figtree-SemiBold.ttf');
    ...
    console.warn('Não foi possível carregar fonte Figtree, usando Helvetica:', error);
    const regularFont = await pdfDoc.embedFont(StandardFonts.Helvetica);
    const boldFont = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
    return { regular: regularFont, bold: boldFont };
  }
...
export async function generateChecklistPDF(
  ...
  try {
    ...
    const { regular: figtreeFont, bold: figtreeBoldFont } = await embedFigtreeFont(pdfDoc);
    ...
    const pontuacaoMedia = notasComValor.length > 0
      ? notasComValor.reduce((acc, nota) => acc + nota, 0) / notasComValor.length
      : 0;
    ...
    const dataFormatada = new Date(resposta.submittedAt || resposta.createdAt).toLocaleString('pt-BR', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
      hour: '2-digit',
    ...
    if (resposta.template.descricao) {
      const descLines = wrapText(resposta.template.descricao, contentWidth, 10, regularFont);
    ...
    // Grupos e perguntas
    for (const grupo of resposta.template.grupos.sort((a, b) => a.ordem - b.ordem)) {
      ...
      if (grupo.descricao) {
        const descLines = wrapText(grupo.descricao, contentWidth - 20, 8, regularFont);
        for (const line of descLines) {
          if (yPos < margin + 50) {
            currentPage = pdfDoc.addPage(pageSize);
            yPos = height - margin;
      ...
      for (const pergunta of perguntasGrupo) {
        ...
        const perguntaLines = wrapText(perguntaText, perguntaColWidth - 20, 9, regularFont);
        ...
        if (pergunta.descricao) {
          const descLines = wrapText(pergunta.descricao, contentWidth - 30, 8, regularFont);
          for (const line of descLines) {
            currentPage.drawText(line, {
              x: margin + 25,
              y: yPos,
        ...
        if (respostaPergunta) {
          if (pergunta.tipo === 'BOOLEANO') {
            ...
            const textoLines = wrapText(`Resposta: ${texto}`, contentWidth - 30, 9, regularFont);
          ...
          if (respostaPergunta.observacao) {
            ...
            currentPage.drawText('Foto não enviada', {
              x: margin + 25,
              y: yPos,
              size: 9,
              font: regularFont,
          ...
          if (respostaPergunta.fotoUrl) {
            ...
            if (pergunta.tipo !== 'FOTO') {
              currentPage.drawText('Fotos anexadas:', {
                x: margin + 25,
                y: yPos,
                size: 9,
                font: regularFont,
            ...
            const espacoNecessario = fotoUrls.length * fotoHeight + 20;
            if (yPos < margin + espacoNecessario) {
              currentPage = pdfDoc.addPage(pageSize);
              yPos = height - margin;
            }
            for (let i = 0; i < fotoUrls.length; i++) {
              ...
              try {
                ...
                console.warn(`Erro ao carregar foto ${i + 1} (${pergunta.titulo}):`, error);
              }
            }
          }
        } else {
          currentPage.drawText('Não respondido', {
            x: margin + 25,
            y: yPos,
            size: 9,
            font: regularFont,
    ...
    if (resposta.observacoes) {
      ...
      const obsLines = wrapText(resposta.observacoes, contentWidth, 10, regularFont);
      for (const line of obsLines) {
        if (yPos < margin + 50) {
          currentPage = pdfDoc.addPage(pageSize);
          yPos = height - margin;
    ...
    if (resposta.assinaturaFotoUrl) {
      try {
        const supervisorImage = await embedImageFromUrl(pdfDoc, resposta.assinaturaFotoUrl);
        if (supervisorImage) {
    ...
    if (resposta.gerenteAssinaturaFotoUrl) {
      ...
      try {
        const gerenteImage = await embedImageFromUrl(pdfDoc, resposta.gerenteAssinaturaFotoUrl);
        if (gerenteImage) {
          const imgWidth = Math.min(
            signatureLineWidth - 10,
